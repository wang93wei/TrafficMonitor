...
本仓库基于上游项目 [zhongyang219/TrafficMonitor](https://github.com/zhongyang219/TrafficMonitor) 进行独立维护，保留原项目功能、许可及文档链接。当前 `backup-master` 分支从上游 `master` 的 `7901ce0` 提交之后，包含以下独立更新：

* 显存显示：新增基于 PDH/DXGI 的“显存”显示项目，支持在主窗口、任务栏窗口、鼠标提示和通知区提示中显示显存已用/总量。
* 显卡选择与显存总量匹配：优先匹配独立显卡，按 PDH 适配器和 DXGI 信息对齐显存总量；无法可靠获取总量时隐藏“显存”项目，避免展示误导数据。
* Win11 任务栏窗口适配：修复副显示器无任务栏图标时的定位兜底，稳定副屏任务栏布局，并减少“显存”项目在任务栏中的多余宽度。
* 硬件监控稳定性：修复硬件监控错误处理可能导致无限弹窗和程序卡死的问题。
* DPI 和显示器变化处理：修复选项设置子页面 DPI 不一致导致控件尺寸不同的问题，并增强分辨率变化后任务栏窗口的重建处理。
* 稳定性与数据安全修复：修复 PNG 皮肤主窗口重绘时的 GDI 句柄泄漏、插件初始化期间获取任务栏窗口句柄的空指针崩溃；历史流量统计改为原子写入，防止读取失败时清空历史记录；网卡连接切换与接口表访问收拢到 UI 线程执行并统一加锁，消除监控线程竞争和退出时的崩溃隐患。
