...
* 显卡显存占用显示：新增基于 PDH/DXGI 的显存占用项目，支持在主窗口、任务栏窗口、鼠标提示和通知区提示中显示显存已用/总量。
* 显卡选择与显存总量匹配：优先匹配独立显卡，按 PDH 适配器和 DXGI 信息对齐显存总量；无法可靠获取总量时隐藏显存占用项目，避免展示误导数据。
* Win11 任务栏窗口适配：修复副显示器无任务栏图标时的定位兜底，稳定副屏任务栏布局，并减少显存占用项目在任务栏中的多余宽度。
* 硬件监控稳定性：修复硬件监控错误处理可能导致无限弹窗和程序卡死的问题。
* DPI 和显示器变化处理：修复选项设置子页面 DPI 不一致导致控件尺寸不同的问题，并增强分辨率变化后任务栏窗口的重建处理。
* 构建与 CI 维护：补充 Lite 解决方案、MSBuild 命令说明，修复 `PdhQuery` 相关 CI 编译问题。
* 项目维护资料：补充 `AGENTS.md`、`IFLOW.md` 及本地开发/测试技能说明，方便后续独立维护。
